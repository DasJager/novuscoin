...
uint256 hashGenesisBlock("0x00000d4174e6ddf6ce6c40a9dbd36d3c655a2f7b766f313a21820367c4f2671b"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // novuscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "novuscoin Signed Message:\n";
...
{
    // novuscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
uint256 static GetOrphanRoot(const CBlockHeader* pblock)
{
 ...
 {
     int64 nSubsidy = 4.94 * COIN;

     if (nHeight == 1)
     {
         nSubsidy = 13000000 * COIN;  // Development fund

     }

         else { nSubsidy = 4.94 * COIN; }

     return nSubsidy + nFees;
 }


 static const int64 nTargetTimespan = 4 * 60 * 60; // novuscoin: 4 Hours
static const int64 nTargetSpacing = 120; // novuscoin: 2 Minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // novuscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // novuscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000001beebe093e5c2203714910d1372f69ec6162ba3ce6dbc8aee4e26104cbb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1485115238;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 530027;

        if (fTestNet)
        {
            block.nTime    = 1485110799;
            block.nNonce   = 829045;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // novuscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("novuscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("novuscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static novuscoinMiner(CWallet *pwallet)
{
    printf("novuscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("novuscoin-miner");
    ...
    try { loop {
        ...
        printf("Running novuscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("novuscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&novuscoinMiner, pwallet));
}
